---
title: "Cleaning and synthesising many-analyst data"
vignette: >
  %\VignetteIndexEntry{Cleaning and synthesising many-analyst data}
...
This vignette is in active development.
...
We have anonymised our dataset using `anonymise_teams()`; which takes a look-up table of new and old identifier names with which to replace each analysis identifier. Our public dataset is `data(ManyEcoEvo)`. The lookup table and de-anonymised dataset is stored in a private component on the project's OSF. We recommend this practice of separating the anonymised and de-anonymised data, to ensure that the identities of analysts remain confidential while adhering to open practices.

## Data Pre-processing for Meta-analysis

Meta-analysis requires that all estimates are standardised to the same scale. This is because the meta-analysis is based on the assumption that the outcome measures are comparable. Note that the ManyAnalysts project utilises two different outcomes for meta-analysis, standardised effect-sizes, or $Z_r$ and out-of-sample predictions $y_i$, But alternative effect-size measures may be utilised instead[^1] for your meta-analysis.
...
```{r}
#| label: demo_standardise_response_Zr
data("ManyEcoEvo")
...
blue_tit_effect_sizes <-
  ...
  pluck("data", 1) %>%
  slice(1:10) %>%
  select(contains("id"), -response_id_S2, contains("beta"), adjusted_df)

blue_tit_effect_sizes

standardise_response(
  dat = blue_tit_effect_sizes,
  estimate_type = "Zr",
  param_table = NULL,
  dataset = "blue tit"
) %>%
...
<!-- #TODO check results for blue_tit_back_tranfsormed: transformation column seems strange. but output from convert_predictions() suggests correct transformation occurred! -->

```{r}
#| label: demo_standardise_response_yi
...
blue_tit_standardised %>%
  pluck("params", 1)
blue_tit_standardised %>%
  pluck("params", 2) # gets a different set depending on the variable

# ---- raw predictions data ----
blue_tit_back_transformed %>%
  pluck("augmented_data", 1)
blue_tit_back_transformed %>%
  pluck("back_transformed_data", 1)
...
-   Standardisation of effect-sizes (fishers' $Z$), however other effect-sizes could be used if necessary [@Gurevitch2018].
...
3.  The estimates are now ready for back-transformation (section @sec-back-transformation) and/or standardisation (section @sec-standardisation).
