...
preprocess_updated_prediction_files <- function(df = data.frame()) {
  ...
  df %>%
    ...
    mutate(updated_submission_data = map(
      .x = `new_csv_file_name--file-submissionID-csv_number`,
      .f = ~ read_submission_data(filepath = here::here("data-raw/analyst_data/S2", .x))
    )) %>% # TODO replace with exported data in pkg / DO NOT hard code in
    group_by(response_id, submission_id, analysis_id, split_id, csv_number) %>%
    mutate(
      updated_checks = map2(
        .x = updated_submission_data,
        .y = dataset,
        .f = ~ validate_predictions(
          data_set = .y,
          input = .x,
          type = "df"
        )
    ...
    dplyr::mutate(failed_checks = map_int(
      .x = updated_checks, # TODO DEV ONLY? or alert if dropped??
