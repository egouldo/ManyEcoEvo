...
fit_uni_mixed_effects <- function(data, N = 5) {
  rlang::check_installed(
    "recipes",
    reason = "to use `recipe()`, `update_role()`, `step_mutate()`, `step_naomit()`"
  )
  rlang::check_installed(
    "parsnip",
  ...
  if (
    !pointblank::test_col_exists(
      data,
      columns = c("mixed_model", starts_with("box_cox_abs_"))
    )
  ) {
    cli::cli_alert_warning(
      c(
        "Columns {.var mixed_model} and ",
        data %>%
    ...
    cli::cli_warn(
      message = c(
        "More than 1 unique value of {.var mixed_model} ",
    ...
    cli::cli_warn(
      message = "Less than {.arg N} = {.val {N}} observations in ",
...
#' @keywords internal
#' @family Model fitting and meta-analysis
poss_fit_uni_mixed_effects <- purrr::possibly(
