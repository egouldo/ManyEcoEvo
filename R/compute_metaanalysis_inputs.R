...
compute_metaanalysis_inputs <- function(.data, estimate_type = character(1L)) {
  ...
  if(estimate_type == "Zr"){
    ...
    .data <- .data %>% 
      ...
      mutate(Zr_VZr = purrr::pmap(.l = list(beta_estimate = beta_estimate, 
                                            beta_SE = beta_SE, 
    ...
    .data <- .data %>% 
      pointblank::col_exists(columns = vars("response_id", 
                                            "submission_id", 
                                            "analysis_id", 
                                            "split_id", 
                                            "augmented_data", 
                                            "link_function",
                                            "response_transformation_description")) %>% #TODO add check for  response transformation
      group_by(response_id, submission_id, analysis_id, split_id) %>% 
      mutate(back_transformed_data = 
               pmap(.l = list(augmented_data, 
                              link_function,
