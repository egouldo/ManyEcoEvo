...
convert_predictions <- function(
  augmented_data,
  transformation_type,
  response_transformation,
  link_fun
) {
  rlang::check_installed("data.table", reason = "to use `setnames()`")
  ...
  if (rlang::is_na(list(augmented_data))) {
    cli::cli_alert_warning(
      "Missing Value for {.arg augmented_data}, returning {.val NA}"
    )
    out <- rlang::na_cpl
  } else if (rlang::is_na(transformation_type)) {
    cli::cli_alert_warning(
      "Missing Value for {.arg transformation_type}, returning {.val NA}"
    )
    out <- rlang::na_cpl
  } else if (!pointblank::has_columns(augmented_data, "se.fit")) {
    # Before Apply Transformation, check valid inputs
    cli::cli_abort(
      message = "Data.frame {.arg augmented_data} must contain column {.val se.fit}"
    )
    ...
    beta_vals <- if (names(key_var) == "scenario") {
      ...
      if (!pointblank::has_columns(augmented_data, "estimate")) {
        cli::cli_abort(
          message = "Blue tit prediction data must contain column {.code estimate}"
        )
      }
      ...
      if (!pointblank::has_columns(augmented_data, "fit")) {
        cli::cli_abort(
          message = "Eucalyptus prediction data must contain column {.code fit}"
        )
      }
      augmented_data$fit
    }
    ...
    converted <- if (transformation_type %nin% "double_transformation") {
      ...
      # Back-transform response AND link-function

      if (any(rlang::is_na(response_transformation), rlang::is_na(link_fun))) {
        cli::cli_alert_warning(
          "Missing Value for {.arg response_transformation}, returning {.val NA}"
        )
        out <- rlang::na_cpl
      }
    ...
    names_lookup <- tibble(
      input_names = augmented_data %>%
        colnames() %>%
        discard(~ .x == colnames(key_var))
    ) %>%
      mutate(
        new_names = case_match(
          input_names,
          "fit" ~ "mean_origin",
      ...
      rows_append(tibble(input_names = "sd.fit", new_names = "sd_origin")) %>%
      deframe()
