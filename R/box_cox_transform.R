...
box_cox_transform <- function(data, dataset, outcome_SE_colname) {
  #TODO undocumented arg
  # ----- Argument Checking -----
  rlang::check_installed("timetk", reason = "to use `step_box_cox()`")
  rlang::check_installed(
    "recipes",
    reason = "to use `prep()`, `tidy()`, `juice()`, recipe()"
  )
  if (any(rlang::is_na(data), rlang::is_null(data))) {
    ...
    cli::cli_h2(
      c(
        "Box-cox transforming absolute deviation scores for ",
    ...
    if (
      ...
      # TODO pull execution of if/else and check result in if() so not executing twice (next line)
      ...
      result <- recipes::juice(box_cox_recipe) %>%
        ...
        mutate(
          fold_params = map2(
            .x = abs_deviation_score_estimate,
