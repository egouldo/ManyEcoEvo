...
compute_MA_inputs <- function(ManyEcoEvo, estimate_type = NULL){
  ...
  if(!rlang::is_null(estimate_type)){
    df <-   ManyEcoEvo  %>% 
      mutate(diversity_indices = map(diversity_data, apply_sorensen_calc),
             effects_analysis = map2(.x = data, .y = diversity_indices, # TODO post-dev, remove data/diversity_data cols??
                                     .f = ~left_join(.x, .y) %>% 
    ...
    df <- ManyEcoEvo %>% 
      ...
      mutate(diversity_indices = map(diversity_data, apply_sorensen_calc),
             effects_analysis = pmap(.l = list(data,  diversity_indices, estimate_type),
                                     .f = ~ left_join(..1, ..2) %>% 
                                       rename(study_id = id_col) %>% 
                                       mutate(estimate_type = ..3))) %>% # TODO can we remove estimate type from this df?
      dplyr::group_by(exclusion_set, dataset, estimate_type)
  }
