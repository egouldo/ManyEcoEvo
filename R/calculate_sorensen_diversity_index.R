...
calculate_sorensen_diversity_index <- function(.data, .id = character()) {
  ...
  out <- out %>%
    ...
    rowwise() %>%
    mutate(
      mean_diversity_index = # TODO why do we take the mean again (remember: these are pairwise comparisons, so we need to reduce down to single value per analysis)
