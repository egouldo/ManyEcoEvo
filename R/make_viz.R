...
make_viz <- function(data) {
  rlang::check_installed("metaviz", reason = "to use `viz_funnel()`")
  rlang::check_installed("broom.mixed", reason = "to use `tidy()`")
  rlang::check_installed("broom", reason = "to use `glance()`")
  rlang::check_installed("parameters", reason = "to use `parameters()`")
  rlang::check_installed("performance", reason = "to use `performance()`")
  # ---- Define Helper Functions ----

  tidy_mod <- function(mod) {
    broom.mixed::tidy(mod, conf.int = TRUE, include_studies = TRUE)
  }
  ...
  viz_out <-
    ...
    mutate(
      ...
      tidy_mod_summary = map_if(
        .x = model,
        .p = ~ !rlang::is_na(.x),
        .f = purrr::possibly(tidy_mod, otherwise = NA, quiet = TRUE),
        .else = ~ return(NA)
      ),
      ...
      mod_glance = map_if(
        .x = model,
        .p = ~ !rlang::is_na(.x),
        .f = purrr::possibly(broom::glance, otherwise = NA, quiet = FALSE),
        .else = ~ return(NA)
      ),
      ...
      forest_plot = ifelse(
        !rlang::is_na(model) & model_name == "MA_mod",
        purrr::pmap(
          .l = list(model, estimate_type, dataset),
          .f = purrr::possibly(gg_forest, otherwise = NA)
        ),
        NA
      ),
      MA_fit_stats = ifelse(
        model_name == "MA_mod" & !rlang::is_na(model),
        map_if(
          .x = model,
          .p = ~ "rma.mv" %in% class(.x),
      ...
      model_params = map_if(
        .x = model,
        .p = possibly(
          \(x) class(x) %in% parameters::supported_models() %>% any(),
          otherwise = NA
        ),
