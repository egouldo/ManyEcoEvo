...
gg_forest <- function(meta_model, estimate_type, dataset = character(1L)) {
  ...
  cli::cli_h2(glue::glue("Creating gg-forest-plot of ", {estimate_type}, " estimates for ", {dataset}, " dataset"))
  ...
  plot_data <- meta_model %>% 
    ...
    tibble::as_tibble() %>% 
    dplyr::mutate(point_shape = ifelse(stringr::str_detect(Parameter, "Overall"), "diamond", "circle"),
                  Parameter = forcats::fct_reorder(Parameter, Coefficient) %>% 
                    forcats::fct_reorder(., point_shape,.desc = TRUE))
  
  x_axis_label <- case_when(estimate_type == "Zr" ~ expression("Standardised Effect Size, Z"[r]),
