...
gg_forest <- function(meta_model, estimate_type, dataset = character(1L)) {
  ...
  plot_data <- meta_model %>%
    ...
    dplyr::mutate(
      point_shape = ifelse(
        stringr::str_detect(Parameter, "Overall"),
        "diamond",
        "circle"
      ),
  ...
  x_axis_label <- case_when(
    #TODO update whether standardised or not
    estimate_type == "Zr" ~ expression("Standardised Effect Size, Z"[r]),
    estimate_type ==
      "ymed" ~ expression("Standardised Out-of-sample Prediction, y"[50]),
    estimate_type ==
      "y25" ~ expression("Standardised Out-of-sample Prediction, y"[25]),
    estimate_type ==
      "y50" ~ expression("Standardised Out-of-sample Prediction, y"[50]),
