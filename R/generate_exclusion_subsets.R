    ...
    df <- subset_fns_df %>% 
      ...
      dplyr::transmute(data = 
                ...
                diversity_data = # this step filters diversity_data according to matches in data, is also applied in prepare_yi
                  purrr::map2(.x = diversity_data, 
                       .y = data,
                       .f = ~ dplyr::semi_join(.x, .y) %>% distinct)) #TODO duplicate cols for euc R_1LRqq2WHrQaENtM, glasgow?
